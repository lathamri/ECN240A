...
reg_test = function(rep, omega, index, covtype){
  ...
  AVSE = sqrt(diag(vcovHC(reg, type=covtype)))[2]
  
  return(list(True = diag(vcovHC(reg, type=covtype))[2], AvgOver = AVSE))#, T_stat = summary(vcovHC(reg))[["coefficients"]][, "t value"][2]))
}

## Parameter Settings
param_list.2 <- expand.grid(rep=1:num_reps, omega = c(1,2), index=c(3,10,25), covtype=list("const", "HC0", "HC2", "HC3"))
