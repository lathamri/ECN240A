...
library(tidyverse)
```
1. We are asked to show that $\hat{\beta}_1$ converges to both a constant and a normal distrbution. This is a function of it being derived using the CLT. Recall that $\hat{\beta}_1 \sim N(\beta, \dfrac{V_{\beta}}n)$, and so as $n \rightarrow \infty$ our variance goes to 0. Below we use show this graphically using non-normal errors and show that they not only approximate $\beta$ but also have variance going to 0, which is exactly what we want.
